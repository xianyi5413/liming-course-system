const { verifyFullBackup } = require("../../src/excel/full_backup");
...
try {
  ...
  const result = verifyFullBackup(input);
  console.log(JSON.stringify({ ok: true, backup_format: result.format, format_version: result.version, counts: result.counts }));
} catch (error) {
