...
function main() {
  const output = path.resolve(argument("--output", path.join(projectRoot, "artifacts", "黎明教育_全量数据_合成验收_v2.xlsx")));
  ...
  try {
    ...
    exportFullData({ dbPath, outputPath: output, appVersion: "synthetic-acceptance-v2", appGitCommit: "synthetic", createdAt: new Date("2026-07-20T05:22:28.000Z") });
