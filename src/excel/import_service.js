...
function templateSourceData(parsed) {
  ...
  data.recharge_records = parsed.recharge_records.map((row, index) => { const businessMonth = monthKeyFromLabel(row.month_label); if (!businessMonth) throw new FullExcelError("FULL_EXCEL_RECHARGE_MONTH_INVALID", `所有充值记录 第${index + 2}行月份格式无效`); return { id: index + 1, student_name: row.student_name, grade: row.grade, prev_actual: 0, prev_gift: 0, cur_recharge: row.cur_recharge, cur_gift: row.cur_gift, recharge_date: row.recharge_date, notes: row.notes, source: "manual", month_key: businessMonth }; });
  data.student_opening_balances = parsed.student_opening_balances.map((row, index) => ({ id: index + 1, month_key: "", student_name: row.student_name, grade: row.grade, opening_actual_balance: row.opening_actual_balance, opening_gift_balance: row.opening_gift_balance, notes: row.notes, created_at: now, updated_at: now }));
