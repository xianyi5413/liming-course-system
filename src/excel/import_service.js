...
const { VISIBLE_SHEET_DEFINITIONS, VISIBLE_SHEET_NAMES, SOURCE_TABLE_DEFINITIONS } = require("./field_definitions");
const {
...
const TEMPLATE_FILENAME = "黎明教育_全量数据导入模板_v2.xlsx";
...
function templateSheet(definition) { return { name: definition.sheet_name, rows: [templateHeaders(definition)], columnWidths: templateHeaders(definition).map(() => 18) }; }
function templateInfoSheet() { return { name: "导出说明", autoFilter: false, columnWidths: [24, 88], rows: [
  ...
  ["年级时间界定", "示例：初三：2025-09-01至2026-06-30；高一：2026-07-01至（空结束日期表示当前阶段）"],
...
function guideSheet() { return { name: TEMPLATE_GUIDE_SHEET, autoFilter: false, columnWidths: [22, 80, 36], rows: [
  ["主题", "填写说明", "合成示例"], ["日期", "日期使用YYYY-MM-DD，月份使用YYYY-MM-01", "2026-01-01"],
  ["金额", "人民币元，填写数值，不带¥或千位逗号", "128.50"], ["课程状态", "使用系统实际状态，例如待上、已上、请假、试课、考试、未缴费", "已上"],
  ["年级阶段", "阶段之间用中文分号；分隔；截止日期可留空表示当前阶段；阶段不得重叠", "初三：2025-09-01至2026-06-30；高一：2026-07-01至"],
...
function parseTimeline(value, studentName) {
  const raw = String(value || "").trim(); if (!raw) return [];
  const result = raw.split("；").map((item) => item.trim()).filter(Boolean).map((item) => { const match = item.match(/^([^：]+)：(\d{4}-\d{2}-\d{2})至(\d{4}-\d{2}-\d{2})?$/); if (!match) throw new FullExcelError("FULL_EXCEL_GRADE_TIMELINE_INVALID", `${studentName}年级时间界定格式无效`); return { student_name: studentName, stage: match[1].trim(), start_date: match[2], end_date: match[3] || "" }; });
  const ordered = [...result].sort((a, b) => a.start_date.localeCompare(b.start_date)); for (let index = 1; index < ordered.length; index += 1) if (ordered[index].start_date <= (ordered[index - 1].end_date || "9999-12-31")) throw new FullExcelError("FULL_EXCEL_GRADE_TIMELINE_OVERLAP", `${studentName}年级阶段日期重叠`);
  return result;
}
...
function templateSourceData(parsed) {
  ...
  let stageId = 1; for (const row of parsed.students) for (const stage of parseTimeline(row.grade_timeline, row.name)) data.student_grade_stages.push({ id: stageId++, ...stage, created_at: now, updated_at: now });
  ...
  data.recharge_records = parsed.recharge_records.map((row, index) => ({ id: index + 1, student_name: row.student_name, grade: row.grade, prev_actual: 0, prev_gift: 0, cur_recharge: row.cur_recharge, cur_gift: row.cur_gift, recharge_date: row.recharge_date, notes: row.notes, source: "manual", month_key: monthKey(row.recharge_date) }));
  data.student_opening_balances = parsed.student_opening_balances.map((row, index) => ({ id: index + 1, month_key: row.month_key, student_name: row.student_name, grade: row.grade, opening_actual_balance: row.opening_actual_balance, opening_gift_balance: row.opening_gift_balance, notes: row.notes, created_at: now, updated_at: now }));
...
function parseTemplate(input) {
  ...
  const info = new Map(workbook.sheetMap.get("导出说明").rows.slice(1).map((row) => [row[0], row[1]])); if (info.get("文件类型") !== TEMPLATE_FILE_TYPE || Number(info.get("格式版本")) !== FORMAT_VERSION) throw new FullExcelError("FULL_EXCEL_FORMAT_INVALID", "模板类型或版本不支持");
...
function normalizeImport(input, options = {}) { const buffer = Buffer.isBuffer(input) ? input : fs.readFileSync(path.resolve(input)); const workbook = validateWorkbookStructure(buffer).workbook; if (workbook.sheetMap.has("__恢复元数据")) return { kind: "full_data", buffer }; const info = workbook.sheetMap.get("导出说明"); const values = new Map((info?.rows || []).slice(1).map((row) => [row[0], row[1]])); if (values.get("文件类型") === TEMPLATE_FILE_TYPE) return { kind: "template", ...templateToFullBuffer(buffer, options) }; throw new FullExcelError("FULL_EXCEL_FORMAT_INVALID", "文件类型不支持"); }
