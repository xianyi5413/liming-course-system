...
test("remote download refuses records without both Baidu fs_id values", async () => {
  const db = new DatabaseSync(databasePath); db.prepare("UPDATE backup_records SET remote_path=?,remote_checksum_path=?,remote_file_id='',remote_checksum_file_id='' WHERE id=?").run("/apps/liming/backup.xlsx", "/apps/liming/backup.xlsx.sha256", backupRecord.id); db.close();
  const response = await fetch(`http://127.0.0.1:${port}/api/data-center/backups/${backupRecord.id}/remote-download`, { headers: { cookie: ownerCookie } }); const result = await response.json();
  assert.equal(response.status, 400); assert.equal(result.code, "BAIDU_REMOTE_FILE_ID_MISSING"); assert.equal(result.error, "该旧备份缺少百度文件ID，无法执行远端下载校验，请重新生成一份百度备份。"); assert.doesNotMatch(JSON.stringify(result), /access_token|dlink|app_secret|cookie|session/i);
});
...
test("backup deletion needs no password or confirmation text but keeps authorization and path protections", async () => {
  ...
  const removed = await fetch(`http://127.0.0.1:${port}/api/data-center/backups/${deletable}`, { method: "DELETE", headers: { cookie: ownerCookie, "content-type": "application/json" }, body: "{}" }); const removedResult = await removed.json(); assert.equal(removed.status, 200, JSON.stringify(removedResult)); assert.equal(removedResult.deleted, true); assert.equal(removedResult.cleanup.local_excel, "already_absent");
  const deletedDetail = await fetch(`http://127.0.0.1:${port}/api/data-center/backups/${deletable}/job`, { headers: { cookie: ownerCookie } }); assert.equal(deletedDetail.status, 404);
  ...
  const checked = new DatabaseSync(databasePath, { readOnly: true }); assert.equal(checked.prepare("SELECT COUNT(*) AS count FROM backup_records WHERE id=?").get(deletable).count, 0); const logs = checked.prepare("SELECT operation_content,extra_json FROM operation_logs WHERE operation_type='删除全量数据备份' ORDER BY id").all(); checked.close();
