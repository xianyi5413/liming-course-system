...
test("amount columns use Excel's two-decimal numeric display style", () => { const sheet = verified.workbook.sheetMap.get("所有学生费用明细"); assert.match(sheet.xml, /<col\b[^>]*min="11"[^>]*style="2"/); assert.match(sheet.xml, /<c r="K2" s="2"><v>199\.5<\/v><\/c>/); assert.match(verified.workbook.entries.get("xl/styles.xml").toString("utf8"), /numFmtId="4"[^>]*applyNumberFormat="1"/); });
...
test("recharge sheet uses source month_key as its first Chinese month column", () => { const sheet = verified.workbook.sheetMap.get("所有充值记录"); assert.deepEqual(sheet.rows[0], ["月份", "学生姓名", "年级", "本月实际充值", "本月赠送充值", "充值日期", "备注"]); assert.equal(sheet.rows[1][0], "2026年4月"); });
