...
before(() => {
  tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "liming-full-excel-v3-")); sourcePath = path.join(tempRoot, "source A", "source.sqlite"); targetPath = path.join(tempRoot, "target B", "target.sqlite"); backupPath = path.join(tempRoot, "output with space", "黎明教育_全量数据_20260720_150000.xlsx");
...
after(() => { if (tempRoot && path.basename(tempRoot).startsWith("liming-full-excel-v3-")) { try { fs.rmSync(tempRoot, { recursive: true, force: true, maxRetries: 5, retryDelay: 100 }); } catch {} } });
...
test("full export uses v3", () => { assert.equal(verified.format, BACKUP_FORMAT); assert.equal(verified.version, FORMAT_VERSION); assert.equal(FORMAT_VERSION, 3); });
...
test("student fee details expose page-order reference fees", () => { const sheet = verified.workbook.sheetMap.get("所有学生费用明细"); assert.deepEqual(sheet.rows[0], ["学生姓名", "授课老师", "日期", "星期", "时间", "教室", "状态", "年级", "科目", "备注", "单人费用", "规则费用"]); assert.deepEqual(sheet.rows[1].slice(-2), [199.5, 188.5]); });
...
test("fee and salary reference columns use the same bucket and empty-rule values as their pages", () => {
  ...
  const pendingFees = feeRows.filter((row) => row[2] === "2026-04-09"); assert.equal(pendingFees.length, 3); assert.equal(pendingFees.every((row) => row[10] === 0 && row[11] === 0), true);
  const completedFees = feeRows.filter((row) => row[2] === "2026-04-10"); const bucketPrice = data.pricing_standards.find((row) => row.grade === "初二" && Number(row.student_count) === 4).unit_price; assert.equal(completedFees.every((row) => row[10] === bucketPrice && row[11] === ""), true);
  assert.deepEqual(salaryRows.find((row) => row[1] === "2026-04-09").slice(-2), [0, 0]); assert.deepEqual(salaryRows.find((row) => row[1] === "2026-04-10").slice(-2), [0, 0]);
});
...
test("student and teacher rule sheets use shared human price statuses", () => { const student = verified.workbook.sheetMap.get("所有学生单价"); const teacher = verified.workbook.sheetMap.get("所有教师薪资规则"); assert.deepEqual(student.rows[0], ["学生", "年级", "科目", "学生集合", "单价", "价格状态", "备注"]); assert.equal(["自动", "手动", "未设置"].includes(student.rows[1][5]), true); assert.deepEqual(teacher.rows[0], ["老师", "年级", "科目", "学生集合", "每2小时薪资", "价格状态", "备注"]); assert.equal(teacher.rows[1][5], "已设置"); assert.doesNotMatch(JSON.stringify([student.rows[0], teacher.rows[0]]), /是否启用/); });
...
test("teacher lesson detail exposes page-order salary references", () => { const sheet = verified.workbook.sheetMap.get("所有教师课时明细"); assert.deepEqual(sheet.rows[0], ["授课老师", "日期", "星期", "时间", "教室", "状态", "年级", "科目", "学生", "备注", "教师薪资", "规则薪资"]); assert.deepEqual(sheet.rows[1].slice(-2), [220.5, 220.5]); });
...
test("v2 full workbook is rejected before v3 column validation with a re-export message", () => { const old = rebuiltWorkbook((sheets) => { sheets.find((sheet) => sheet.name === "导出说明").rows.find((row) => row[0] === "格式版本")[1] = 2; }); assert.throws(() => verifyFullData(old), (error) => error.code === "FULL_EXCEL_FORMAT_INVALID" && /v3/.test(error.message)); });
...
test("synthetic acceptance fixture uses the global opening-balance schema", () => { const output = path.join(tempRoot, "acceptance fixture", "黎明教育_全量数据_合成验收_v3.xlsx"); const result = spawnSync(process.execPath, [path.join(root, "scripts/excel_backup/create_acceptance_fixture.js"), "--output", output], { cwd: root, encoding: "utf8" }); assert.equal(result.status, 0, result.stderr); const fixture = verifyFullData(output); assert.equal(fixture.data.student_opening_balances.length, 1); assert.equal(Object.prototype.hasOwnProperty.call(fixture.data.student_opening_balances[0], "month_key"), false); });
