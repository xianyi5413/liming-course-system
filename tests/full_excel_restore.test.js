...
const { BACKUP_FORMAT, FORMAT_VERSION, buildFullDataBufferFromSourceData, exportFullData, verifyFullData, restoreFullData, expectedSheetNames, SETTING_LABELS } = require("../src/excel/full_backup");
...
function seedCompleteData(db) {
  db.exec(`
    ...
    INSERT INTO teacher_salary_rules(id,teacher_name,grade,subject,student_names,salary_per_unit,unit_hours,is_active,notes,created_at,updated_at) VALUES (502,'恢复教师','初二','英语','恢复学生',333,2,0,'停用技术状态仅隐藏恢复','2026-01-01','2026-04-01');
    ...
    INSERT INTO lessons(id,teacher_name,date,lesson_status,time_slot,classroom,grade,subject,student_names,notes,course_status,status,teacher_salary,teacher_salary_source,teacher_salary_rule_id,month_key,sort_order,created_at,updated_at) VALUES (701,'恢复教师','2026-04-08','上课','09:00-11:00','A1','初二','数学','恢复学生','=公式文本😀','已上','已上',220.5,'rule',501,'2026-04-01',7,'2026-04-01 08:00:00','2026-04-08 12:00:00');
    INSERT INTO fee_overrides(lesson_id,student_name,unit_price,updated_at) VALUES (701,'恢复学生',199.5,'2026-04-08 12:01:00');
    INSERT INTO recharge_records(id,student_name,grade,prev_actual,prev_gift,cur_recharge,cur_gift,recharge_date,notes,source,channel,channel_other,month_key) VALUES (801,'恢复学生','初二',100,50,2000,200,'2026-04-03','充值备注','manual','other','线下转账','2026-04-01');
    INSERT INTO recharge_records(id,student_name,grade,prev_actual,prev_gift,cur_recharge,cur_gift,recharge_date,notes,source,month_key) VALUES (802,'恢复学生','初二',0,0,10,0,'','空日期充值','manual','2026-05-01');
    INSERT INTO student_opening_balances(id,student_name,grade,opening_actual_balance,opening_gift_balance,notes,created_at,updated_at) VALUES (901,'恢复学生','初二',100,50,'期初备注','2026-03-31','2026-04-01');
...
after(() => { if (tempRoot && path.basename(tempRoot).startsWith("liming-full-excel-v4-")) { try { fs.rmSync(tempRoot, { recursive: true, force: true, maxRetries: 5, retryDelay: 100 }); } catch {} } });
...
test("student fee details keep actual fee and omit recalculable rule fee", () => { const sheet = verified.workbook.sheetMap.get("所有学生费用明细"); assert.deepEqual(sheet.rows[0], ["学生姓名", "授课老师", "日期", "星期", "时间", "教室", "状态", "年级", "科目", "备注", "单人费用"]); assert.equal(sheet.rows[1][10], 199.5); assert.equal(sheet.rows[0].includes("规则费用"), false); });
test("amount cells use the two-decimal style without styling the whole column", () => { const sheet = verified.workbook.sheetMap.get("所有学生费用明细"); assert.doesNotMatch(sheet.xml, /<col\b[^>]*min="11"[^>]*style="2"/); assert.match(sheet.xml, /<c r="K2" s="2"><v>199\.5<\/v><\/c>/); assert.match(verified.workbook.entries.get("xl/styles.xml").toString("utf8"), /numFmtId="4"[^>]*applyNumberFormat="1"/); });
test("actual fee and salary columns keep page values without recalculable duplicates", () => {
  ...
  const completedFees = feeRows.filter((row) => row[2] === "2026-04-10"); const bucketPrice = data.pricing_standards.find((row) => row.grade === "初二" && Number(row.student_count) === 4).unit_price; assert.equal(completedFees.every((row) => row[10] === bucketPrice && row.length === 11), true);
  assert.equal(salaryRows.find((row) => row[1] === "2026-04-09")[10], 0); assert.equal(salaryRows.find((row) => row[1] === "2026-04-10")[10], 0); assert.equal(salaryRows.every((row) => row.length === 11), true);
});
test("recharge sheet includes structured channel fields after the source month", () => { const sheet = verified.workbook.sheetMap.get("所有充值记录"); assert.deepEqual(sheet.rows[0], ["月份", "学生姓名", "年级", "本月实际充值", "本月赠送充值", "充值日期", "来源/渠道", "其他渠道说明", "备注"]); assert.equal(sheet.rows[1][0], "2026年4月"); });
test("recharge month survives an empty recharge date and rows remain month ordered", () => { const rows = verified.workbook.sheetMap.get("所有充值记录").rows.slice(1); assert.deepEqual(rows.map((row) => row[0]), ["2026年4月", "2026年5月"]); assert.equal(rows[1][5], ""); assert.equal(verified.data.recharge_records.find((row) => row.id === 802).month_key, "2026-05-01"); });
test("structured recharge channel survives full export and restore data", () => { const row = verified.data.recharge_records.find((item) => item.id === 801); assert.equal(row.channel, "other"); assert.equal(row.channel_other, "线下转账"); });
test("opening balances expose one global business row without month", () => assert.deepEqual(verified.workbook.sheetMap.get("期初余额").rows[0], ["学生姓名", "年级", "期初实际余额", "期初赠送余额", "备注"]));
test("opening-balance recovery mapping contains no legacy month_key", () => {
  const rows = verified.workbook.sheetMap.get("__关系映射").rows.slice(1);
  assert.equal(rows.some((row) => row[2] === "student_opening_balances" && row[4] === "month_key"), false);
  assert.equal(Object.prototype.hasOwnProperty.call(verified.data.student_opening_balances[0], "month_key"), false);
  const description = verified.workbook.sheetMap.get("导出说明").rows.find((row) => row[0] === "期初余额");
  assert.deepEqual(description, ["期初余额", "每名学生一条全局基础余额；可见表和隐藏恢复映射均不含月份"]);
});
test("student and teacher rule sheets use only shared set or unset statuses", () => { const student = verified.workbook.sheetMap.get("所有学生单价"); const teacher = verified.workbook.sheetMap.get("所有教师薪资规则"); assert.deepEqual(student.rows[0], ["学生", "年级", "科目", "学生集合", "单价", "价格状态", "备注"]); assert.equal(student.rows[1][5], "已设置"); assert.deepEqual(teacher.rows[0], ["老师", "年级", "科目", "学生集合", "每2小时薪资", "价格状态", "备注"]); assert.deepEqual(teacher.rows.slice(1).map((row) => row[5]).sort(), ["已设置", "未设置"]); assert.doesNotMatch(JSON.stringify([student.rows, teacher.rows]), /已停用|是否启用|true|false/); });
test("hidden recovery mapping preserves inactive teacher-rule technology state", () => { const mapping = verified.workbook.sheetMap.get("__关系映射").rows.slice(1); assert.equal(mapping.some((row) => row[2] === "teacher_salary_rules" && row[4] === "is_active"), true); assert.equal(verified.data.teacher_salary_rules.find((row) => row.id === 502).is_active, 0); });
...
test("teacher lesson detail keeps actual salary and omits recalculable rule salary", () => { const sheet = verified.workbook.sheetMap.get("所有教师课时明细"); assert.deepEqual(sheet.rows[0], ["授课老师", "日期", "星期", "时间", "教室", "状态", "年级", "科目", "学生", "备注", "教师薪资"]); assert.equal(sheet.rows[1][10], 220.5); assert.equal(sheet.rows[0].includes("规则薪资"), false); });
...
test("operation logs can be excluded while preserving the v4 sheet contract and metadata", () => {
...
test("student grade stages are expanded into thirteen visible date columns", () => { const sheet = verified.workbook.sheetMap.get("学生档案"); assert.equal(sheet.rows.length, 2); assert.deepEqual(sheet.rows[0].slice(2, 15), ["初一起始日期", "初一截止日期", "初二起始日期", "初二截止日期", "初三起始日期", "初三截止日期", "高一起始日期", "高一截止日期", "高二起始日期", "高二截止日期", "高三起始日期", "高三截止日期", "已毕业日期"]); assert.deepEqual(sheet.rows[1].slice(4, 8), ["2025-09-01", "2026-06-30", "2026-07-01", ""]); });
...
test("v3 full workbook is rejected before v4 column validation with a re-export message", () => { const old = rebuiltWorkbook((sheets) => { sheets.find((sheet) => sheet.name === "导出说明").rows.find((row) => row[0] === "格式版本")[1] = 3; }); assert.throws(() => verifyFullData(old), (error) => error.code === "FULL_EXCEL_FORMAT_INVALID" && /v4/.test(error.message)); });
...
test("restoring a workbook without operation logs preserves target logs", () => {
  const output = path.join(tempRoot, "without logs restore", "全量数据_不含操作日志.xlsx"); const target = path.join(tempRoot, "without logs restore target", "target.sqlite");
  exportFullData({ dbPath: sourcePath, outputPath: output, includeOperationLogs: false }); initDatabase(target);
  const before = new DatabaseSync(target); before.prepare("INSERT INTO operation_logs(operation_type,operation_content,result_status) VALUES ('目标日志','必须保留','success')").run(); const count = before.prepare("SELECT COUNT(*) AS count FROM operation_logs").get().count; before.close();
  restoreFullData({ dbPath: target, inputPath: output }); const after = new DatabaseSync(target, { readOnly: true }); assert.equal(after.prepare("SELECT COUNT(*) AS count FROM operation_logs").get().count, count); assert.equal(after.prepare("SELECT COUNT(*) AS count FROM operation_logs WHERE operation_content='必须保留'").get().count, 1); after.close();
});
test("restored original rules and actual values support fee and salary recalculation", () => { const db = new DatabaseSync(targetPath, { readOnly: true }); const studentRule = db.prepare("SELECT custom_price FROM student_pricing WHERE id=401").get(); const override = db.prepare("SELECT unit_price FROM fee_overrides WHERE lesson_id=701 AND student_name='恢复学生'").get(); const teacherRule = db.prepare("SELECT salary_per_unit,is_active FROM teacher_salary_rules WHERE id=501").get(); const lesson = db.prepare("SELECT teacher_salary,teacher_salary_rule_id FROM lessons WHERE id=701").get(); db.close(); assert.deepEqual({ rule_fee: studentRule.custom_price, actual_fee: override.unit_price, rule_salary: teacherRule.salary_per_unit, active: teacherRule.is_active, actual_salary: lesson.teacher_salary, rule_id: lesson.teacher_salary_rule_id }, { rule_fee: 188.5, actual_fee: 199.5, rule_salary: 220.5, active: 1, actual_salary: 220.5, rule_id: 501 }); });
