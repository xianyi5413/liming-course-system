...
function filledTemplate() {
  ...
  add("学生档案", ["模板学生", "初一", "初一：2026-01-01至2026-06-30；初二：2026-07-01至", "模板家长", "13800000001", "在读", "2026-01-01", "", "学生备注"]);
  add("费用标准", ["初一", 1, 300, "初一-1", "1对1"]); add("所有学生单价", ["模板学生", "初一", "数学", "模板学生", 188.5, "规则"]);
  add("所有教师薪资规则", ["模板教师", "初一", "数学", "模板学生", 200, "启用", "薪资规则"]); add("所有班级管理", ["模板教师", "初一", "数学", "模板学生", "模板班"]);
  add("所有课程数据", ["模板教师", "2026-04-08", "周三", "09:00-11:00", "A1", "已上", "初一", "数学", "模板学生", "课程"]);
  add("所有充值记录", ["模板学生", "初一", 1000, 100, "2026-04-03", "充值"]); add("期初余额", ["2026-04-01", "模板学生", "初一", 100, 50, "期初"]);
...
before(() => { tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "liming-template-v2-")); targetPath = path.join(tempRoot, "target", "target.sqlite"); templatePath = path.join(tempRoot, TEMPLATE_FILENAME); initDatabase(targetPath); fs.writeFileSync(templatePath, filledTemplate()); });
after(() => { if (tempRoot && path.basename(tempRoot).startsWith("liming-template-v2-")) { try { fs.rmSync(tempRoot, { recursive: true, force: true, maxRetries: 5, retryDelay: 100 }); } catch {} } });
...
test("filled template preview succeeds", () => { const result = previewImport(templatePath); assert.equal(result.ok, true); assert.equal(result.kind, "template"); assert.equal(result.format_version, 2); });
...
test("template with an overlapping grade timeline is rejected", () => { const broken = path.join(tempRoot, "overlap.xlsx"); fs.writeFileSync(broken, mutate(filledTemplate(), (sheets) => { sheets.find((sheet) => sheet.name === "学生档案").rows[1][2] = "初一：2026-01-01至2026-08-01；初二：2026-07-01至"; })); assert.throws(() => previewImport(broken), (error) => error.code === "FULL_EXCEL_GRADE_TIMELINE_OVERLAP"); });
...
test("template filename advertises v2", () => assert.match(TEMPLATE_FILENAME, /_v2\.xlsx$/));
