...
test("template initialization restores one global opening balance with no month column", () => { const db = new DatabaseSync(targetPath, { readOnly: true }); assert.equal(db.prepare("SELECT COUNT(*) AS count FROM student_opening_balances WHERE student_name='模板学生'").get().count, 1); assert.equal(db.prepare("PRAGMA table_info(student_opening_balances)").all().some((column) => column.name === "month_key"), false); db.close(); });
