...
- 涉及页面：课程总表、周课表、学生费用明细、学生费用汇总、充值记录、期初余额、学生查询、学生单价、学生档案、薪资规则、教师档案。
- 自检方法：启动项目并登录后，逐页检查表头、金额列、备注列、学生列、checkbox 列和日期/时间列宽；同时确认课程新增/编辑/筛选/批量删除、费用汇总、充值记录、期初余额导入导出、学生查询导出/复制图片、薪资规则和档案编辑等核心功能没有异常。
...
- 继续补齐页面金额展示的人民币符号：`学生 -> 费用明细` 的单人费用 / 规则费用、`学生 -> 充值记录` 的本月实际充值 / 本月赠送充值、`学生 -> 期初余额` 的期初实际余额 / 期初赠送余额、`学生 -> 学生单价` 的单价、`教师 -> 教师薪资` 的车票 / 课时合计 / 薪资合计、`教师 -> 薪资明细` 的教师薪资 / 规则薪资、`教师 -> 薪资规则` 的每2小时薪资、`设置 -> 费用标准` 的单人费用。
