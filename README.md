...
- 列宽优化：课程总表和周课表的日期、时间列按 `YYYY-MM-DD`、`HH:mm-HH:mm` 收紧；课程总表学生列适当加宽；备注列不再无意义过宽；统一表格不再强制 `min-width: 100%` 平均撑满。
- 涉及页面：课程总表、周课表、学生费用明细、学生费用汇总、充值记录、期初余额、学生查询、学生单价、学生档案、课时明细、教师档案。
...
- 批量操作按钮禁用态：费用明细“按规则更新所选费用”、充值记录“批量删除”、期初余额“批量删除”、薪资明细“按规则更新所选薪资”、教师档案“批量删除”统一为未选择时灰色禁用，勾选至少一条可处理记录后恢复可用；禁用态使用真实 `disabled`，不会弹确认框或发请求。
- 充值记录批量删除：修复后端 `uniqueSorted is not defined`，批量删除会在事务内去重排序受影响月份、删除充值流水、刷新后续月份结转，并继续写入审计日志和操作日志；删除后前端刷新数据，费用汇总和余额按剩余充值重新计算。
- 表格空白优化：周课表和学生费用明细使用专用 `compact-table-scroll`，取消左右稳定 gutter；checkbox 列紧凑居中，日期、状态、星期、时间、教室、年级、科目等短列收紧，学生列保留适度宽度，备注列限制在合理范围，金额列右对齐、表头居中。
- 新增期初余额弹窗：弹窗宽度调整到约 640px，表单改为两列布局，学生姓名、年级、期初实际余额、期初赠送余额对齐展示，备注独占整行；窄屏自动切换单列，底部取消/保存按钮保持右对齐。
- 自检方法：逐页确认上述批量按钮未勾选时灰色禁用、勾选后恢复；充值记录批量删除不再出现 `uniqueSorted is not defined`；周课表/费用明细左右白色空白消失或明显减少；新增期初余额弹窗自然排开，保存、默认金额 0、学生/年级选择仍正常。
...
`教师 -> 薪资明细` 中，非 `已上` 课程薪资为 0。教师薪资右上角用 `自 / 手 / 未` 标识来源状态，并支持勾选后点击 `按规则更新所选薪资`。独立来源列和学生人数列已删除；手动薪资不会被自动覆盖，除非用户主动批量按规则更新。
...
批量复制弹窗支持整体平移天数，默认 `+7` 天；上方显示原课程，下方显示复制后课程。原课程只读，目标课程可编辑。复制后的课程不继承原教师薪资，而是按新增课程逻辑重新匹配课时明细规则。
...
3. **角色预筛选**
   `role_filter_presets` 保存角色在课程总表、周课表、矩阵课表等页面的隐形基础数据池。预筛选不再等同于页面筛选框默认值；用户页面筛选属于二次筛选，最终数据按“角色预筛选 ∩ 用户页面筛选”返回。
4. **绑定老师**
...
- 后端派生数据使用内存缓存；写入课程、充值、期初余额、价格规则、课时明细规则等数据后会统一清理缓存，避免脏数据。
...
| `teacher_salary_rules` | 老师 + 年级 + 科目 + 学生集合的课时明细规则，金额为每 2 小时薪资 |
...
| 教师 | 线性图标 | 薪资汇总、薪资明细、课时明细、车费明细、xlsx 导出 | `teachers` / `teacher_salary_rules` / `teacher_adjustments_monthly` / `teacher_travel_fees` |
...
| 周课表 / 矩阵课表 | 前端按自然周和日期范围展示 | 矩阵课表提供时间课表、老师课表、教室课表三视图，当前仅用于查看课程；冲突检查入口已迁移到课程总表 |
...
| 子功能 | 实现 | 说明 |
| --- | --- | --- |
| 薪资汇总 | `teacherSummary(monthKey)` | 输出：有效课时数 / 课时合计 / 动态周车费 / 备注；汇总页只读，不直接编辑车费 |
| 薪资明细 | 选老师 → 当月课程明细 | 表底显示合计课时费 |
| 课时明细 | `teacher_salary_rules` | 维护老师 + 年级 + 科目 + 学生集合的课时薪资自动匹配规则 |
...
- 复制后的教师薪资不继承原课程，仍按当前新增课程 / 课时明细规则逻辑处理。
...
- `教师 -> 薪资明细` 中，课程状态不是“已上”的记录教师薪资按 0 处理，不参与课时明细规则匹配，也不能被批量“按规则更新所选薪资”。
- 新增、复制、编辑课程时，如果状态不是“已上”，不会自动填教师薪资。
- 月度核心 Excel 导出复用同一套费用和薪资计算逻辑，因此非“已上”课程在导出中也按上述口径处理。

#### 课时明细规则

`教师 -> 课时明细` 用于维护教师课时薪资的自动计算规则。规则匹配条件为：
...
- 新增课程时，如果命中有效课时明细规则，会自动填入教师薪资。
- 复制课程时，不继承原课程薪资，而是按当前规则重新匹配。
- 没有命中规则时，薪资留空，等待人工核对。
- 已有历史课程薪资不会因为新增规则而自动修改。
- 手动薪资永远优先；`teacher_salary = 0` 可以是合法人工值，不会被当作空值自动覆盖。

`教师 -> 薪资明细` 增加了规则薪资参考和来源判断，来源大致包括：`无规则`、`待填写`、`自动`、`手动`。当前薪资与规则计算值一致时显示为自动；不一致时显示为手动。薪资明细支持勾选课程后点击“按规则更新所选薪资”，只会把所选课程更新为当前规则计算值，未选中的课程不会被修改；没有有效规则或规则金额为 0 的课程不能批量应用。
...
进入 `教师 -> 课时明细` 页面时，系统会根据历史课程自动补齐规则候选。候选规则默认金额为 0，只作为待设置候选显示，不参与自动匹配；当“每2小时薪资”大于 0 时，规则才参与新增课程自动匹配、复制课程重新匹配、薪资明细规则薪资计算和批量按规则更新。课时明细页已简化：不显示状态列和操作列，不需要保存 / 启用 / 停用按钮，修改“每2小时薪资”或备注后失焦自动保存。

课时明细页面优化：

- `+ 新增课时明细` 按钮移动到全局规则卡片标题附近，新增规则使用弹窗。
...
- 最新入口已迁移到课程总表，周课表和矩阵课表不再显示冲突检查 UI。
...
- 继续补齐页面金额展示的人民币符号：`学生 -> 费用明细` 的单人费用 / 规则费用、`学生 -> 充值记录` 的本月实际充值 / 本月赠送充值、`学生 -> 期初余额` 的期初实际余额 / 期初赠送余额、`学生 -> 学生单价` 的单价、`教师 -> 薪资汇总` 的车票 / 课时合计 / 薪资合计、`教师 -> 薪资明细` 的教师薪资 / 规则薪资、`教师 -> 课时明细` 的每2小时薪资、`设置 -> 费用标准` 的单人费用。
...
- 教师栏目命名调整为 `薪资汇总`、`薪资明细`、`课时明细`、`车费明细`。`薪资汇总` 页面只读展示，不再直接编辑车费；导出本月功能保留。
...
- 周课表和矩阵课表删除冲突检查 UI，不再显示冲突检查面板、冲突数量标签、忽略教室 1 开关或冲突列表；两页保留周切换、筛选、周课表明细和矩阵三视图展示。
- 自检建议：检查日期范围未选中布局、日历图标和 hover 清空；检查日期面板可连续点击两天生成范围；检查课程总表搜索框 Enter 后失焦且 hover 可清空；检查筛选统计区域靠右；检查课程总表冲突按钮和弹窗；检查周课表 / 矩阵课表已无冲突检查区域。
