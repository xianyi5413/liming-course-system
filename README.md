...
- 课程总表默认展示本周课程，并新增“今日 / 本周 / 本月”快捷日期筛选；手动修改日期后会保留用户自定义范围。
- 顶部操作区统一放置“排课模式 / 整周复制 / 新增课程”按钮；“整周复制”和“新增课程”继续复用原有功能，普通模式下保持常驻显示。
- 新增“排课模式”，该模式只显示排课辅助操作，不改变课程总表原有行内编辑能力。
- 排课模式下，课程总表会在不同老师、不同日期的课程块后显示加号行，最后一个课程块后也会显示。
- 点击加号不弹窗，直接新增一条课程记录，并自动带入当前老师、当前日期和“待上”状态。
- 修正课程总表统计口径：标题下方显示当前月份总览；上方统计卡片显示当前筛选结果，并随日期、老师、学生、状态、搜索条件变化。
