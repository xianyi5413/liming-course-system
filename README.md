...
- 期初余额支持通过 Excel 导入，并可下载导入模板；后续准备补充期初余额导出 Excel 功能。
...
- 顶部月份切换当前会触发全量 `load()`；同时批次 FIFO 学生扣费使费用汇总、学生查询等派生计算更重，因此月份切换可能出现等待。
- 后续性能优化方向包括：前端缓存学生档案、老师档案、期初余额和规则等全局数据；月份切换时只刷新月度数据；后端缓存费用汇总、扣费和余额等派生结果，并在相关数据写入后失效。
