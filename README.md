...
- 新增期初余额弹窗：弹窗宽度调整到约 640px，表单改为两列布局，学生姓名、年级、期初现金余额、期初赠送余额对齐展示，备注独占整行；窄屏自动切换单列，底部取消/保存按钮保持右对齐。
...
- 页面表格只展示本月实际充值、本月赠送学费、充值日期和备注等本月流水信息；“上月实际结转 / 上月赠送结转”仅在核心 Excel 导出中保留，不再显示在充值记录 UI。
