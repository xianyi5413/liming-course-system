...
`教师 -> 课时明细` 中，非 `已上` 课程薪资为 0。教师薪资右上角用 `自 / 手 / 未` 标识来源状态，并支持勾选后点击 `按规则更新所选薪资`。独立来源列和学生人数列已删除；手动薪资不会被自动覆盖，除非用户主动批量按规则更新。
...
课程总表日期筛选已统一为日期范围选择器，输入框展示 `开始日期 → 截止日期`；筛选区保留 `本周 / 下周 / 本月` 三个常用快捷按钮，`本周` 和 `下周` 按当前真实日期计算周一至周日，`本月` 按顶部当前系统月份计算首日至末日。切换顶部月份不会强制重置课程总表的日期筛选。
...
- 家长群课程截图个人发送覆盖所有学生；预览模式保持原卡片预览，简洁模式按个人发送 / 班级发送和年级分区展示小矩形，小矩形点击轮流复制截图和文案，复制文案后变绿。
...
6. **本轮新增功能权限口径**
   学生年级阶段单行保存和批量修改接口归属 `学生档案(studentProfiles)` 页面权限，并继续受只读状态限制；班级管理权限保持不变。家长群截图的预览 / 简洁模式切换、课程总表 `本周 / 下周 / 本月` 快捷按钮、班级管理和薪资规则的 `隐藏非在职老师` 开关都不新增权限。
...
| 矩阵课表 | 全局课程数据按日期范围展示 | 提供时间课表、老师课表、教室课表三视图，保留日期范围、重置、回到本周和视图切换；不受顶部月份限制，冲突检查入口已迁移到课程总表 |
| 家长群课程截图 | 页面「排课 / 家长群课程截图」+ `/api/course-notice` | 按日期范围直接读取数据库课程，支持日常班 / 暑期班截图模式、生成发送对象、修改称呼、复制文案、复制/下载课程截图、完成打勾和清除所有打勾记录 |
...
| 车费明细 | `teacher_travel_fees` + `teacher_adjustments_monthly` | 可编辑老师每月各周车费；保存写入操作日志，并同步旧 1-4 周兼容字段 |
| 课时明细 | 选老师 → 当月课程明细 | 表底显示合计课时费，支持复制图片和按规则更新所选薪资 |
...
- 课程总表默认展示本周课程，日期筛选统一为带左侧快捷时间和右侧双月日历的日期范围选择器；旧的独立“昨日 / 今日 / 明日 / 上周 / 本周 / 下周 / 上月 / 本月 / 下月”按钮已移除，快捷时间仍按真实自然日期计算。
...
- 起止日期筛选统一为日期范围选择器，展示为 `YYYY-MM-DD → YYYY-MM-DD`；左侧提供本月、本周、上月、上周、近三个月、近一年、去年、今年，右侧为双月日历，支持单箭头切换月份、双箭头切换年份、先选起始或先选截止、禁用不可选日期、hover 预览和实心选中范围。
- 已替换的日期范围包括：首页课程趋势、课程总表、矩阵课表、学生费用明细、学生查询、经营概览、日常开销、操作日志、家长群课程截图和老师课程截图；单日期输入如新增课程日期、充值日期、档案入离职日期仍保持单日期选择器。
- 课程总表、家长群课程截图、老师课程截图中的独立快捷日期按钮已移除，快捷时间整合进日期范围选择器左侧；日期计算函数保留给新控件复用。
...
- 矩阵课表新增三种视图：时间课表、老师课表、教室课表。时间课表保留原有时间段矩阵逻辑；老师课表按老师 × 日期展示课程；教室课表按教室 × 日期展示课程，三种视图共用当前周切换、日期范围和老师 / 学生 / 教室 / 年级 / 科目 / 状态筛选结果。
- 自检建议：逐页检查普通筛选框默认箭头、展开搜索图标、已选 hover 小 `×`、Enter 后 blur；检查日期范围面板快捷时间、双月日历、月份 / 年份切换、先选起始或截止、hover 预览和清空；检查课程总表筛选区不跳位、排课态和浏览态时间 / 状态视觉一致；检查矩阵课表三种视图切换、周切换和筛选条件均生效。
...
- 新增 `教师 -> 车费明细` 页面，可按当前月份编辑每位老师各周车费。保存走 `PUT /api/teacher-travel-fees`，写入 `teacher_travel_fees`，同步 `teacher_adjustments_monthly` 旧 1-4 周字段，并记录操作日志。
...
- 学生档案表格行高、输入框高度、padding 和字号继续向教师档案收敛，避免学生档案行高偏高。
...
- 筛选框右侧图标统一为固定图标区：普通下拉箭头、展开后的搜索图标、日期日历图标和 hover 清空 `×` 均放大并垂直居中；清空 `×` 使用浅灰圆形背景，图标与圆形中心对齐。
- 课程总表搜索框和部分纯文本筛选框改为通用文本筛选控件：输入后按 Enter 会提交并 `blur`，hover 有值的搜索框时出现灰底小 `×`，点击后清空并触发原筛选刷新逻辑。
- 各筛选区的统计和重置区域统一右对齐：`已筛选 x / 共 x 条` 与 `重置 / 清空筛选` 按钮整体靠右，窄屏时换行后仍尽量靠右显示。
- 课程总表状态列改为普通文字显示：浏览态不再使用状态 badge 颜色或背景；排课态状态下拉仍可编辑，但课程总表内覆盖为普通文字视觉，不影响状态含义和保存。
- 冲突检查迁移到课程总表：按钮放在 `批量删除` 后方，按当前筛选结果实时计算冲突数量；`冲突为0` 显示绿色且不可点击，有冲突时显示红色 `冲突为N` 并可打开 `时间冲突检查` 弹窗。
- 冲突弹窗保留 `忽略教室为 1 的教室冲突` 开关，并显示老师 / 学生 / 教室 / 时间格式冲突统计；每条冲突展示日期、星期、时间、授课老师、教室、年级、科目、学生和状态，可定位到相关课程。
- 矩阵课表删除冲突检查 UI，不再显示冲突检查面板、冲突数量标签、忽略教室 1 开关或冲突列表；页面保留日期范围、重置、回到本周和矩阵三视图展示。旧周课表页面已下线，历史入口会回到课程总表 / 矩阵课表。
...
- 矩阵课表顶部保留日期范围、重置、回到本周和 `时间课表 / 老师课表 / 教室课表` 三个视图，不再显示快捷周按钮。
...
- 家长群课程截图新增 `日常班截图 / 暑期班截图` 切换；暑期班按 `老师 + 科目 + 年级` 聚合发送对象。
...
- 家长群截图新增 `预览模式 / 简洁模式`。预览模式保持原卡片预览；简洁模式分为 `个人发送` 和 `班级发送` 两个区域，并按初一至高三分区展示小矩形。
- 简洁模式小矩形点击会在 `复制截图` 和 `复制文案` 之间轮流执行；复制文案成功后小矩形变绿，`清除所有打勾记录` 会同时清除简洁模式绿色状态。
- `排课 -> 课程总表` 在日期范围旁保留 `本周 / 下周 / 本月` 三个快捷按钮；`本月` 使用顶部当前系统月份的首日至末日。
