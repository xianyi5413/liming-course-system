...
学生集合会做去空、去重、排序和统一分隔符处理。页面只维护班级名；若班级名存在，家长群课程截图和老师课程截图中学生列会显示为 `班级名：学生`。
...
课程总表日期筛选已统一为日期范围选择器，输入框展示 `开始日期 → 截止日期`；左侧集成本月、本周、上月、上周、近三个月、近一年、去年、今年等快捷时间，右侧使用双月日历选择范围。快捷时间以当前真实日期为基准，`本月` 指当前真实自然月，不是顶部选择的月份；切换顶部月份不会强制重置课程总表的日期筛选。
...
| 矩阵课表 | 前端按日期范围展示 | 提供时间课表、老师课表、教室课表三视图，保留日期范围、重置、回到本周和视图切换；冲突检查入口已迁移到课程总表 |
...
| 矩阵课表 | 支持时间课表、老师课表、教室课表三种视图；时间课表保留原时间段矩阵和长范围稀疏卡片逻辑，老师课表按老师 × 日期展示，教室课表按教室 × 日期展示；冲突检查 UI 已迁移到课程总表 |
