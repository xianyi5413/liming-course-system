...
- 当前后端诊断版本为 `2026.06.26-ui-perf-batch-1`，前端入口脚本为 `/app.js?v=20260626-ui-perf-batch-1`。
...
本轮新增三类批量删除，均支持勾选、多选、二次确认、删除后刷新和审计/操作日志：
...
删除前请先用当前数据做备份，尤其是充值记录和教师档案。充值流水会影响费用汇总、学生余额、经营概览和后续月结转；教师档案若已有历史课程，不会硬删课程关联。
...
6. **本轮新增功能权限口径**
   学生年级阶段单行保存和批量修改接口归属 `学生档案(studentProfiles)` 页面权限，并继续受只读状态限制；班级管理权限保持不变。家长群截图的预览 / 简洁模式 / 班级合并发送、课程总表 `今日 / 本周 / 下周 / 本月` 快捷按钮、班级管理和薪资规则的 `隐藏非在职老师` 开关都不新增权限。全部月份核心 Excel zip 和备份查看 / 下载 / 设置归属核心导出能力。
...
- 已接入学生费用按规则更新、教师课时薪资按规则更新、教师车费保存、学生档案阶段保存 / 批量界定时间、学生 / 教师档案新增编辑删除
...
- 权限口径：年级阶段单人保存和批量保存沿用 `学生档案` 页面权限；家长群截图模式切换、课程总表快捷日期和隐藏非在职老师开关不新增权限。
